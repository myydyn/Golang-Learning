...
class Item{
    ...
    //     return `${this.id}-${this.spec.name}-${this.spec.type}-${this.spec.color}-${this.imgUrl}-${this.price}-${this.quantity}`;
    // }
...
class DigitalItemSpec extends ItemSpec{
    ...
    compare(item: DigitalItemSpec): boolean {
        return super.compare(item) && // do not forget to call parent method
